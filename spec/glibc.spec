#%define glibcsrcdir  glibc-2.20-201411
%define glibcsrcdir glibc-linaro-2.20-2014.11
...
%define rtkaioarches %{ix86} ppc %{power64} s390 s390x
##############################################################################
...
# glibc bugfix remalloc when segmentfault .
Patch0054: %{name}-fix-malloc-deadlock-bug.patch
...
Patch5000: %{name}-clear-PTHREAD_MUTEX_NO_ELISION_NP-flag.patch
Patch5001: %{name}-CVE-2017-16997-elf-Check-for-empty-tokens-before-dynamic-string-tok.patch
Patch5002: %{name}-CVE-2015-5180-resolv-Fix-crash-with-internal-QTYPE-B.patch
Patch5003: %{name}-2.20-CVE-2017-12132-resolv-Reduce-EDNS-payload.patch
...
The GNU libc libraries embedded
...
#      -e '\,/%{_lib}/lib\pcprofile\.so,d' \
#      -e '\,bin/\(mtrace\|xtrace\|pcprofiledump\),d'
#} | sort > rpm.filelist
...
%endif
%ifarch aarch64
ln -sf /lib64/ld-linux-aarch64.so.1 $RPM_BUILD_ROOT/lib/ld-linux-aarch64.so.1
%endif
...
%defattr(-,root,root)
%exclude /sbin/ldconfig
%exclude /%{_lib}/libpthread*.so*
%exclude /%{_lib}/libc-*.so*
%exclude /%{_lib}/libc.so.*
%exclude /%{_lib}/ld-*.so*
%exclude /%{_lib}/libdl*.so*
%exclude /%{_lib}/libm-*.so*
%exclude /%{_lib}/libm.so.*
%exclude /%{_lib}/libresolv*.so*
%exclude /%{_lib}/libnss_files*.so*
%exclude /%{_lib}/librt-*.so*
%exclude /%{_lib}/librt.so.*
%exclude /%{_lib}/libcrypt*
%exclude /%{_lib}/libutil*
